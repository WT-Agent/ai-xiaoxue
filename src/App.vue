  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">适用年级</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">辅导诉求</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">小学全科辅导与亲子家校共育指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 小学全科辅导与习惯养成专家');
...
const metricsList = [
  {
    "key": "elementaryPedagogyClarity",
    "label": "小学教学通俗度"
  },
  {
    "key": "habitFormationGuidance",
    "label": "习惯养成引导性"
  },
  {
    "key": "parentChildInteractionRatio",
    "label": "亲子互动促进分"
  },
  {
    "key": "mistakeDiagnosisPrecision",
    "label": "错题诊断精准度"
  },
  {
    "key": "learningInterestIncentive",
    "label": "学习兴趣激发度"
  }
];
const guideRules = [
  {
    "title": "用具象事物替代抽象概念",
    "advice": "解释数学分段计费或分数时，用切苹果、分饼干等具象实物进行对比演示",
    "avoid": "切忌直接灌输抽象复杂的代数方程"
  },
  {
    "title": "先肯定努力再指出错处",
    "advice": "批改辅导时采用“三体赞美法”：先夸字迹工整或思路正确部分，再引导修正错误",
    "avoid": "避免大声呵斥“你怎么这么笨”打击孩子自信"
  },
  {
    "title": "控制单次学习时长",
    "advice": "低年级专注力只有 15-20 分钟，建议每学习 20 分钟休息 5 分钟做眼保健操",
    "avoid": "严禁强迫孩子连续做作业 2 小时不上厕所"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[XIAOXUE_SCORES\][\s\S]*?\[\/XIAOXUE_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[XIAOXUE_SCORES\](.*?)\[\/XIAOXUE_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('xiaoxue_history_records');
...
const saveHistory = () => {
  localStorage.setItem('xiaoxue_history_records', JSON.stringify(historyList.value));
};
